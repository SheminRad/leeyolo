...
MODEL_PATH = "/home/shemin/PycharmProjects/leeyolo/runs/detect/robust/detect/lee_yolo_robust/weights/best.pt"
DATASET_YAML = "japan.yaml"
...
def save_table_as_image(model_name, dataset, params, gflops, latency, fps, map50, map50_95, precision, recall, conf):
    ...
    plt.title(f"Robust Benchmarking Results: {model_name}\nDataset: {dataset}", pad=20, weight='bold')

    safe_name = model_name.replace(" ", "_").replace("(", "").replace(")", "").lower()
    filename = f"Robust_{dataset_name}_metrices.png"
