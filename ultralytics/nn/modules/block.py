...
class GSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
        c_ = c2 // 2
        # FIX: Explicitly label kwargs here as well
        self.cv1 = Conv(c1, c_, k, s, p=p, g=g, act=act)
        self.cv2 = Conv(c_, c_, 5, 1, p=None, g=c_, act=act)

    def forward(self, x):
        x1 = self.cv1(x)
        x2 = torch.cat((x1, self.cv2(x1)), 1)
        return channel_shuffle(x2, 2)

class SGC2f(nn.Module):
    # Lightweight SGC2f block replacing C2f
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = GSConv(c1, 2 * self.c, 1, 1)
        self.cv2 = GSConv((2 + n) * self.c, c2, 1)  # optional act=False
        self.m = nn.ModuleList(GhostBottleneck(self.c, self.c, 3, 1) for _ in range(n))
...
class ECA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3):
        super().__init__()
        # c1 and c2 are accepted to satisfy the Ultralytics parser
...
class EMA(nn.Module):
    ...
    def __init__(self, c1, c2, groups=8):
        ...
        # FIX: Add a true 2D Global Average Pool for Path 2
        self.pool_2d = nn.AdaptiveAvgPool2d(1)

        self.conv1x1 = nn.Conv2d(c1 // groups, c1 // groups, 1)
        self.conv3x3 = nn.Conv2d(c1 // groups, c1 // groups, 3, padding=1)
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, x):
        b, c, h, w = x.size()
        # Divide into G sub-feature groups
        x_grouped = x.view(b * self.groups, c // self.groups, h, w)

        # Path 1: Cross-spatial learning
        x_h = self.pool_h(x_grouped)
        x_w = self.pool_w(x_grouped).permute(0, 1, 3, 2)
        hw = self.conv1x1(torch.cat([x_h, x_w], dim=2))
        x_h, x_w = torch.split(hw, [h, w], dim=2)
        a1 = self.sigmoid(x_h) * self.sigmoid(x_w.permute(0, 1, 3, 2))

        # Path 2: Global spatial information
        x_3x3 = self.conv3x3(x_grouped)

        # FIX: Use pool_2d here instead of pool_h so H and W are both pooled to 1
        a2 = self.softmax(self.pool_2d(x_3x3).view(b * self.groups, -1))

        # Merge parallel outputs
        out = (x_grouped * a1) * a2.view(b * self.groups, c // self.groups, 1, 1)
